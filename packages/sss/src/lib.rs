use lagrange::{lagrange_interpolation_at_zero, evaluate_polynomial, sample_polynomial};
...
pub use ec_curve::secp256k1::scalar::ScalarElement;
pub use share::Share;
...
pub fn split<T: ECScalar, R: CryptoRng + RngCore>(rng: &mut R, data: &T, k: u8, n: u8) -> Vec<Share<T>> {

    let poly = sample_polynomial(k as usize, data, rng);
    let indexes: Vec<u8> = (1..=n).collect();
    let secret_shares = evaluate_polynomial(&poly, &indexes);

    let shares: Vec<Share<T>> = secret_shares.iter()
        .enumerate()
        .map(|(index, share)| {
            Share {
                id: (index + 1) as u32,
